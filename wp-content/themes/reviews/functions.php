...
if( !function_exists('reviews_check_plugins') ){
function reviews_check_plugins() {
	...
	if( function_exists('sc_activate') ):
	    ...
	    if( $smeta_data['social-connect.php']['Version'] != '1.4' ):
		    ?>
		    <div class="notice notice-success is-dismissible error">
...
add_action( 'admin_notices', 'reviews_check_plugins' );
}
...
function reviews_enqueue_font( $font_family ) {
    $font_url = '';
    $font_family = str_replace( '+', ' ', $font_family );
    if ( 'off' !== _x( 'on', 'Google font: on or off', 'reviews' ) ) {
        $font_url = add_query_arg( 'family', urlencode( $font_family.':400,300,700&subset=all' ), "//fonts.googleapis.com/css" );
    }
    return $font_url;
}
}
...
function reviews_scripts_styles(){
	...
	if( !empty( $title_font ) ){
		$loaded_fonts[] = $title_font;
		wp_enqueue_style( 'reviews-title-font', reviews_enqueue_font( $title_font ), array(), '1.0.0' );
	}
	$navigation_font = reviews_get_option( 'navigation_font' );
	if( !empty( $navigation_font ) && !in_array( $navigation_font, $loaded_fonts ) ){
		$loaded_fonts[] = $navigation_font;
		wp_enqueue_style( 'reviews-navigation-font', reviews_enqueue_font( $navigation_font ), array(), '1.0.0' );
	}
	$text_font = reviews_get_option( 'text_font' );
	if( !empty( $text_font ) && !in_array( $text_font, $loaded_fonts ) ){
		wp_enqueue_style( 'reviews-text-font', reviews_enqueue_font( $text_font ), array(), '1.0.0' );
	}
...
if( !function_exists('reviews_link_pages') ){
function reviews_link_pages( $post_pages ){
...
if( !function_exists('reviews_format_pagination') ){
function reviews_format_pagination( $page_links ){
	$list = '';
	if( !empty( $page_links ) ){
...
function reviews_send_contact(){
	...
	if( empty( $name ) || empty( $subject ) || empty( $email ) || empty( $message ) ){
		$response = array(
			'error' => esc_html__( 'All fields are required.', 'reviews' ),
		);
	}
...
function reviews_review_category( $icon_only = false, $post_id = '' ){
	...
	if( !empty( $review_categories ) ){
		...
		if( !$icon_only ){
			echo '<a href="'.esc_url( add_query_arg( array( $reviews_slugs['review-category'] => $term->slug ), $permalink ) ).'">'.$term->name.'</a>';
		}
		else{
			...
			if( !empty( $main ) ){
				...
				echo '<a title="'.esc_attr( $term->name ).'" href="'.esc_url( add_query_arg( array( $reviews_slugs['review-category'] => $term->slug ), $permalink ) ).'"><i class="fa fa-'.esc_attr( $icon ).'"></i></a>';
			}
		}
	}
...
function reviews_display_children( $taxonomy, $term_id, $permalink, $number = '' ){
	...
	if( !empty( $terms ) ){		
		foreach( $terms as $term ){
			$list .= '<a href="'.esc_url( add_query_arg( array( $reviews_slugs['review-category'] => $term->slug ), $permalink ) ).'" class="clearfix">
...
function reviews_display_children_all_cat( $taxonomy, $children, $permalink, $permalink_cats, $num_subs ){
	...
	foreach( $children as $child ){
		...
		$list .= '<a href="'.esc_url( add_query_arg( array( $reviews_slugs['review-category'] => $child->slug ), $has_children ? $permalink_cats : $permalink ) ).'" class="clearfix">
...
function reviews_list_categories_li( $terms ){
	...
	$list = '';
	foreach( $terms as $term ){
		$list .= '<li><a href="'.esc_url( add_query_arg( array( $reviews_slugs['review-category'] => $term->slug ), reviews_get_permalink_by_tpl( 'page-tpl_search' ) )  ).'">'.$term->name.'</a></li>';
...
function reviews_display_categories_filter( $ancestors = array(), $permalink = '', $parent = 0, $depth = 0 ){
	...
	if( !empty( $categories ) ){
		foreach( $categories as $category ){
			...
			$option .= str_repeat('&nbsp;&nbsp;', $depth).$category->name.' ('.$category->count.')';
			$option .= '</option>';
			echo  $option;
...
function reviews_single_review_slider_images( $image_id ) {
	...
	echo '<a href="'.$full_image_data[0].'">
			<img class="reviews-lazy-load" src="'.esc_url( get_template_directory_uri().'/images/holder.jpg' ).'" data-src="'.esc_url( $post_thumbnail_data[0] ).'" width="'.esc_attr( $post_thumbnail_data[1] ).'" height="'.esc_attr( $post_thumbnail_data[2] ).'" alt="">
		  </a>';
}
}
